export default function PopupWithForm(props) {
    return (
        ...
        <section className={`popup popup-${props.name} ${props.isOpen ? 'popup_is-opened' : ''} `}>
          <form className={`popup__form popup__form-${props.name}`} name="popupForm" noValidate>
            ...
            {props.children}
            <button type="submit" className="popup__submit-button popup__submit-button_form">Сохранить</button>
          </form>
        </section>
    )
}